// 아래 SECRET_KEY를 이용해 토큰을 생성하는 모듈.
...
exports.handler = async (event, cxt, callback) => {
    ...
    try{
        ...
        var auth = body.id
        if (auth === "admin") {
            var claim = {"id":auth, "role":"admin", "ts": Date.now() + 600000}; // 10min token
            const seed = JSON.stringify(claim) + SECRET_KEY
            const sha = crypto.createHash('sha1')
            const shad = sha.update(seed).digest('hex')
            const token = Base64.encode(algo) + '.' 
                        + Base64.encode(JSON.stringify(claim)) + '.'
                        + shad
            return {
            statusCode: 200,
